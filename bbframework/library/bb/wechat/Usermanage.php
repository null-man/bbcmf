...
class UserManage{
     ...
     */
    public static function createGroup($groupName, $public_id){
        //获取ACCESS_TOKEN
    ...
    // 批量获取用户基本信息
    public static function getBatchUserInfo($userlist, $public_id) {
        //获取ACCESS_TOKEN
        $accessToken = AccessToken::getAccessToken($public_id);
        $queryUrl = 'https://api.weixin.qq.com/cgi-bin/user/info/batchget?access_token='.$accessToken;
        $data = '{"user_list":'.json_encode($userlist).'}';
        return Curl::callWebServer($queryUrl, $data, 'POST');
    }
     ...
     */
    public static function setRemark($openId, $remark, $public_id){
        //获取ACCESS_TOKEN
        $accessToken = AccessToken::getAccessToken($public_id);
     ...
     * linjy 添加
     */
    public static function deleteGroup($group_id, $public_id){
        //获取ACCESS_TOKEN
        $accessToken = AccessToken::getAccessToken($public_id);
