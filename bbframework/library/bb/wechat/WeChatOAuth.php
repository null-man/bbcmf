...
class WeChatOAuth{
     ...
     */
    public static function getCode($redirect_uri, $public_id, $scope='snsapi_base', $state=1){
        if($redirect_uri[0] == '/'){
            $redirect_uri = substr($redirect_uri, 1);
        }
        //公众号的唯一标识
//        $appid = WECHAT_APPID;
        $appid = Weixin::get_appid($public_id);
...
//        $redirect_uri = WECHAT_URL . $redirect_uri;
     ...
     */
    public static function getAccessTokenAndOpenId($code, $public_id){
        //填写为authorization_code
        $grant_type = 'authorization_code';
        //构造请求微信接口的URL
        $publicinfo = Weixin::get_publicinfo($public_id);
        $url = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid='.$publicinfo['app_id'].'&secret='.$publicinfo['app_secret'].'&code='.$code.'&grant_type='.$grant_type.'';
     ...
     */
    public static function refreshToken($refreshToken, $public_id){
        $publicinfo = Weixin::get_publicinfo($public_id);
        $queryUrl = 'https://api.weixin.qq.com/sns/oauth2/refresh_token?appid='.$publicinfo['app_id'].'&grant_type=refresh_token&refresh_token='.$refreshToken;
        $queryAction = 'GET';
